...
LOCAL_CONFIG="OpenSiri-debug.local.xcconfig"
...
migrate_legacy_setup() {
  git update-index --no-assume-unchanged OpenSiri-debug.xcconfig \
    2>/dev/null || true
  git update-index --no-skip-worktree OpenSiri-debug.xcconfig \
    2>/dev/null || true

  sed -i '' -E \
    "s/DEVELOPMENT_TEAM *= *.*/DEVELOPMENT_TEAM =/" \
    OpenSiri-debug.xcconfig
  sed -i '' -E \
  ...
  sed -i '' -E \
    ...
    OpenSiri-debug.xcconfig
  if ! grep -Fqx '#include? "OpenSiri-debug.local.xcconfig"' \
    OpenSiri-debug.xcconfig; then
    printf '\n#include? "OpenSiri-debug.local.xcconfig"\n' \
      >> OpenSiri-debug.xcconfig
  fi
