# OpenSiri release workflow

OpenSiri releases are orchestrated by `asc workflow`. The workflow keeps the
...
`release-opensiri-legacy.sh`. It is available as a temporary fallback and is
...
```bash
./scripts/release/release-opensiri.sh release 2.22.0
```

The default channel is `beta`. For a stable update:

```bash
./scripts/release/release-opensiri.sh release 2.22.0 --channel stable
```
...
./scripts/release/release-opensiri.sh prepare 2.22.0

# Prepare, synchronize the release refs, and create a verified GitHub draft.
./scripts/release/release-opensiri.sh draft 2.22.0

# Publish an existing verified draft, install the appcast, and verify remotely.
./scripts/release/release-opensiri.sh publish 2.22.0
```
...
```bash
./scripts/release/release-opensiri.sh release 2.22.0 --dry-run
```
...
```bash
./scripts/release/release-opensiri.sh resume <run-id>
```
...
- `release-opensiri.sh`: stable command-line entry point.
...
but the normal OpenSiri release should not need flags beyond version, channel,
and notes.
