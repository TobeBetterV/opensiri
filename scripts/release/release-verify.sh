...
verify_local_appcast() {
    python3 "$SCRIPT_DIR/release-appcast.py" validate \
        ...
        --download-url "$(release_download_prefix)OpenSiri.zip"
}

verify_zip_contents() {
    local extracted_app="$RELEASE_VERIFY_DIR/OpenSiri.app"
...
verify_asset_urls() {
    ...
    for asset_name in OpenSiri.zip OpenSiri.dmg SHA256SUMS.txt; do
