...
# Shared paths, configuration, and safety helpers for the OpenSiri release
...
RELEASE_REPOSITORY="${RELEASE_REPOSITORY:-TobeBetterV/opensiri}"
...
RELEASE_PROJECT_PATH="OpenSiri.xcodeproj"
RELEASE_WORKSPACE_PATH="OpenSiri.xcworkspace"
RELEASE_SCHEME="OpenSiri"
RELEASE_TARGET="OpenSiri"
...
if [[ -n "$RELEASE_VERSION" ]]; then
    ...
    RELEASE_ARCHIVE_PATH="$RELEASE_DIR/OpenSiri.xcarchive"
    RELEASE_DERIVED_DATA="$RELEASE_DIR/derived-data"
    RELEASE_EXPORT_DIR="$RELEASE_DIR/export"
    RELEASE_APP_PATH="$RELEASE_EXPORT_DIR/OpenSiri.app"
    RELEASE_ARTIFACT_DIR="$RELEASE_DIR/artifacts"
    RELEASE_ZIP_PATH="$RELEASE_ARTIFACT_DIR/OpenSiri.zip"
    RELEASE_DMG_PATH="$RELEASE_ARTIFACT_DIR/OpenSiri.dmg"
    RELEASE_CHECKSUM_PATH="$RELEASE_ARTIFACT_DIR/SHA256SUMS.txt"
    RELEASE_NOTARY_ZIP="$RELEASE_DIR/OpenSiri-notarization.zip"
...
release_notes_url() {
    printf 'https://github.com/TobeBetterV/opensiri/releases/tag/%s' \
        "$RELEASE_VERSION"
}

release_appcast_url() {
    printf 'https://raw.githubusercontent.com/TobeBetterV/opensiri/main/appcast.xml'
}
