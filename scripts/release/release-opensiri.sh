...
# Public entry point for preparing, drafting, and publishing OpenSiri releases.
...
usage() {
    cat <<'EOF'
Usage:
  release-opensiri.sh prepare <version> [options]
  release-opensiri.sh draft <version> [options]
  release-opensiri.sh publish <version> [options]
  release-opensiri.sh release <version> [options]
  release-opensiri.sh resume <run-id>
...
The legacy release implementation remains available as:
  scripts/release/release-opensiri-legacy.sh
EOF
}
