...
- 新增或修改了 `OpenSiriTests/**/*.swift` 下的测试源码。
- 用户明确要求构建或测试。
...
xcodebuild test-without-building \
  -workspace OpenSiri.xcworkspace \
  -scheme OpenSiri \
  -only-testing:OpenSiriTests/<TestSuiteOrClass> | xcbeautify

# Run one test method
xcodebuild test-without-building \
  -workspace OpenSiri.xcworkspace \
  -scheme OpenSiri \
  -only-testing:OpenSiriTests/<TestSuiteOrClass>/<testMethod> | xcbeautify
```
