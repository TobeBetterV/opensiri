...
- `plutil -lint OpenSiri.xcodeproj/project.pbxproj`、`git diff --check`：通过。
- 实际无密钥 oneshot 请求：HTTP 200，返回译文“测试”。
- 定向 Xcode 测试：被现有 `KeyCombo` 编译错误及缺失 Masonry headers 阻断，未发现 DeepL 独立诊断。

### Affected files

- `OpenSiri/Swift/Service/DeepL/DeepLService+Translate.swift`
- `OpenSiri/Swift/Service/DeepL/DeepLTranslateResponse.swift`
- `OpenSiriTests/Service/DeepLServiceTests.swift`
