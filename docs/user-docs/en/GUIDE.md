# OpenSiri Complete Usage Guide

> This document contains detailed feature descriptions, configuration methods, and usage tips for OpenSiri.

## Table of Contents

- [OpenSiri Complete Usage Guide](#opensiri-complete-usage-guide)
...
- [x] Support [🍎 Apple System Dictionary](./How-to-use-macOS-system-dictionary-in-OpenSiri.md), support third-party dictionaries with manual mdict dictionaries import functionalities.
- [x] Support macOS system translation. (_Please see [How to use 🍎 macOS system translation in OpenSiri?](./How-to-use-macOS-system-translation-in-OpenSiri.md)_)
...
The latest version of OpenSiri supports macOS 13.0+, if the system version is macOS 11.0+, please use [2.7.2](https://github.com/TobeBetterV/opensiri/releases/tag/2.7.2).

### Manual Installation

[Download](https://github.com/TobeBetterV/opensiri/releases) the latest release of the app.
...
1. Download this Repo, and then open the `OpenSiri.xcworkspace` file with [Xcode](https://developer.apple.com/xcode/) (⚠️⚠️⚠️ Note that it is not `OpenSiri.xcodeproj` ⚠️⚠️⚠️).
...
If you prefer not to use the script, create `OpenSiri-debug.local.xcconfig` with the following contents. This file is already ignored by Git:
...
Once OpenSiri is launched, in addition to the main window (hidden by default), there will be a menu icon, and clicking on the menu option will trigger the corresponding actions, as follows:

<div>
  <img src="https://github.com/Jerry23011/OpenSiri/assets/89069957/f0c7da85-b9e0-4003-b673-e93f6477a75b" width="50%" />
</div>
...
| Silent Screenshot OCR     | Press the Silent Screenshot shortcut key（default `⌥ + ⇧ + S`）to capture the area, the OCR results will be copied directly to the clipboard | ![屏幕录制 2023-05-20 22 39 11](https://github.com/Jerry23011/OpenSiri/assets/89069957/c16f3c20-1748-411e-be04-11d8fe0e61af)                    |
...
<div>
    <img src="https://github.com/Jerry23011/OpenSiri/assets/89069957/a1d8aa6b-69d7-459a-ac83-a6f090d04cae" width="45%">
    <img src="https://github.com/Jerry23011/OpenSiri/assets/89069957/4dbf038b-d939-454f-9205-648636f46ca8" width="45%">
</div>
...
For detailed information, please see [How to use macOS system dictionary in OpenSiri](./How-to-use-macOS-system-dictionary-in-OpenSiri.md)
...
If you don't have your own OpenAI APIKey, you can use some open source projects to convert third-party LLM interfaces to standard OpenAI interfaces, so that you can use them directly in `OpenSiri`.
...
> [!IMPORTANT]
> [2.6.0](https://github.com/TobeBetterV/opensiri/releases) version implements a new SwiftUI settings page (macOS 13+ support), which supports configuring the service API key in a GUI way, other system verions need to be configured using commands in OpenSiri's input box.
...
The way to customize the DeepL API URL is equivalent to the DeepL official AuthKey API form in OpenSiri.

OpenSiri supports the [DeepLX](https://github.com/OwO-Network/DeepLX) API, see [#464](https://github.com/tisfeng/Easydict/issues/464) for details.
...
OpenSiri supports fast lookup for URL scheme: `opensiri://query?text=xxx`, such as `opensiri://query?text=good`.

If the query content xxx contains special characters, URL encoding is needed, such as `opensiri://query?text=good%20girl`.

> [!WARNING]
> The old version of opensiri://xxx may cause problems in some scenarios, so it is recommended to use the complete URL Scheme:
> opensiri://query?text=xxx

### Use with PopClip

You need to install [PopClip](https://pilotmoon.com/popclip/) first, then select the following code block, `PopClip` will show "Install Extension OpenSiri", just click it.
...
tell application "OpenSiri"
  launch
  open location "opensiri://query?text={popclip text}"
end tell
```
...
<img width="1036" alt="Prefences" src="https://github.com/Jerry23011/OpenSiri/assets/89069957/7d63ad8e-927f-44e2-bc14-9d2199a927e4">

### Services

OpenSiri has 3 types of Windows and you can set different services for each of them.
...
OpenSiri has some in-app shortcuts to help you use it more efficiently.

Unlike the translation shortcut keys that are globally effective, the following shortcuts only take effect when the OpenSiri window is in the foreground.
...
<div style="display:flex;align-items:center;">
  <img src="https://github.com/Jerry23011/OpenSiri/assets/89069957/584bb1b3-6ddd-4af8-a8b5-fc491a21605c" style="margin-right:50px;" width="40%">
  <img src="https://github.com/Jerry23011/OpenSiri/assets/89069957/9f9d99c3-ca07-48dd-9892-ac7fe595a981" width="30%">
</div>
...
- [Raycast-OpenSiri](https://github.com/tisfeng/Raycast-OpenSiri): My other open source project, a Raycast extension version of OpenSiri.

![opensiri-1-1671806758](https://raw.githubusercontent.com/tisfeng/ImageBed/main/uPic/opensiri-1-1671806758.png)
...
As a developer and beneficiary of a lot of open source software, I think that there should be a free open source version of [Bob](https://github.com/ripperhe/Bob) in the world, so I made [OpenSiri](https://github.com/TobeBetterV/opensiri).

Now I use OpenSiri a lot every day, I like it very much, and I hope more people can know it and use it.
