...
OpenSiri uses Xcode String Catalog to manage translations, so the following steps are what you need to get started on localizing the app.
### Installing Xcode 15+
...
1. Navigate to `OpenSiri -> OpenSiri -> App -> Localizable.xcstrings`. Also Expand `Main.storyboard` to find `Main.xcstrings (Strings)`. These two `.xcstrings` files are what you are going to work on.
...
1. Find OpenSiri's icon on the top toolbar of Xcode and click on it
...
- [Sample Pull Request for OpenSiri](https://github.com/tisfeng/Easydict/pull/668)
