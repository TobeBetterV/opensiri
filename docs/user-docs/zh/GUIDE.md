...
> 本文档包含 OpenSiri 的详细功能说明、配置方法和使用技巧。

## 目录

- [OpenSiri 完整使用指南](#opensiri-完整使用指南)
  - [目录](#目录)
  - [详细功能列表](#详细功能列表)
  - [安装指南](#安装指南)
    - [手动下载安装](#手动下载安装)
...
- [x] 支持 [🍎 苹果系统词典](./How-to-use-macOS-system-dictionary-in-OpenSiri.md)，支持第三方词典，可手动导入 mdict 词典。
- [x] 支持 macOS 系统翻译。详情请看 [如何在 OpenSiri 中使用 🍎 macOS 系统翻译？](./How-to-use-macOS-system-translation-in-OpenSiri.md)
...
OpenSiri 最新版本支持系统 macOS 13.0+，如果系统版本为 macOS 11.0+，请使用 [2.7.2](https://github.com/TobeBetterV/opensiri/releases/tag/2.7.2)。
...
1. 下载这个 Repo，然后使用 [Xcode](https://developer.apple.com/xcode/) 打开 `OpenSiri.xcworkspace` 文件（⚠️⚠️⚠️ 注意不是 `OpenSiri.xcodeproj` ⚠️⚠️⚠️）。
2. 使用 `Cmd + R` 编译运行即可。
...
如果不想使用脚本，可以创建 `OpenSiri-debug.local.xcconfig` 并写入以下内容。该文件已经被 Git 忽略：
...
OpenSiri 启动之后，除了应用主界面（默认隐藏），还会有一个菜单图标，点击菜单选项即可触发相应的功能，如下所示：
...
OpenSiri 自动支持词典 App 中系统自带的词典，如牛津英汉汉英词典（简体中文-英语），现代汉语规范词典（简体中文）等，只需在词典 App 设置页启用相应的词典即可。

另外，苹果词典也支持自定义导入词典，因此我们可以通过导入 .dictionary 格式的词典来添加第三方词典，如简明英汉字典，朗文当代高级英语辞典等。

详情请看 [如何在 OpenSiri 中使用 🍎 macOS 系统词典？](./How-to-use-macOS-system-dictionary-in-OpenSiri.md)
...
如果你没有自己的 OpenAI APIKey，可以借助一些开源项目将第三方的 LLM 接口转为标准的 OpenAI 接口，这样就能直接在 `OpenSiri` 中使用了。
...
> [!IMPORTANT]
> [2.6.0](https://github.com/TobeBetterV/opensiri/releases) 版本实现了新的 SwiftUI 设置页（支持 macOS 13+），支持 GUI 方式配置服务 API key，其他系统版本则需要在 OpenSiri 的输入框中使用命令方式配置。
...
使用自定义 DeepL 接口地址的方式，在 OpenSiri 程序中等同于 DeepL 官方 AuthKey API 形式。

OpenSiri 支持 [DeepLX](https://github.com/OwO-Network/DeepLX) 接口，详情请看 [#464](https://github.com/tisfeng/Easydict/issues/464)。
...
OpenSiri 支持 URL scheme 快速查询：`opensiri://query?text=xxx`，如 `opensiri://query?text=good`。 

如果查询内容 xxx 包含特殊字符，需进行 URL encode，如 `opensiri://query?text=good%20girl`。 

> [!WARNING]
> 旧版本的 opensiri://xxx 在某些场景下可能会出现问题，因此建议使用完整的 URL Scheme:
> opensiri://query?text=xxx

### 配合 PopClip 使用

你需要先安装 [PopClip](https://pilotmoon.com/popclip/)，然后选中以下代码块，`PopClip` 会显示 "安装扩展 OpenSiri"，点击它即可。
...
OpenSiri 有 3 种窗口类型，可以分别为它们设置不同的服务。
...
OpenSiri 有一些应用内快捷键，方便你在使用过程中更加高效。

不同于前面的翻译快捷键全局生效，下面这些快捷键只在 OpenSiri 窗口前台显示时生效。
...
<div style="display:flex;align-items:flex-start;">
  ...
  <img src="https://github.com/Jerry23011/OpenSiri/assets/89069957/274adbc6-8391-4386-911c-241db4a1bd98" width="30%">
</div>
...
- [Raycast-OpenSiri](https://github.com/tisfeng/Raycast-OpenSiri): 我的另一个开源项目，一个 Raycast 扩展版本的 OpenSiri。
...
作为一名开发者，也是众多开源软件的受益者，我觉得，这世界上应该存在一个免费开源版本的 [Bob](https://github.com/ripperhe/Bob)，于是我开发了 [OpenSiri](https://github.com/TobeBetterV/opensiri)。现在，我每天都在大量使用 OpenSiri，我很喜欢它，也希望能够让更多的人了解它、使用它。
