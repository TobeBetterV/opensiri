...
OpenSiri 是一个 macOS 翻译和词典应用，正在进行从 Objective-C 到 Swift + SwiftUI 的逐步迁移。
...
- **Outcome**：
  ...
  - 更新 ObjC 调用方以导入 `OpenSiri-Swift.h`。
...
- **Outcome**：
  ...
  - 更新 `EZBaseQueryViewController.m` 以导入 `OpenSiri-Swift.h`。
  - 更新 `OpenSiri-Bridging-Header.h`，注释掉旧的 Objective-C 导入。
...
1. **EZDetectManager** ✅ - 文本检测管理器
   ...
   - **成果**:
     ...
     - 更新了 `EZBaseQueryViewController.m` 导入 `OpenSiri-Swift.h`
     - 更新了 `OpenSiri-Bridging-Header.h` 注释掉旧的 Objective-C 导入
...
- [GitHub Repository](https://github.com/TobeBetterV/opensiri)
