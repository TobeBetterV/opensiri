...
将 OpenSiri 的 DeepL Web 翻译从已失效的 `www2.deepl.com/jsonrpc` 迁移到
...
- OpenSiri 使用 Alamofire，Web 和官方 Key 路径共享同一个服务类。
...
- `plutil -lint OpenSiri.xcodeproj/project.pbxproj`：通过。
- `git diff --check`：通过。
- 手动 HTTP 验证：oneshot endpoint 返回 HTTP 200，`translations[0].text` 为“测试”。
- `xcodebuild test -only-testing:OpenSiriTests/DeepLServiceTests`：未完成；Xcode beta
