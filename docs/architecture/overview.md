...
OpenSiri 是一款 macOS 词典和翻译应用，支持直接查词、文本翻译、划词翻译、
...
OpenSiriTests/                   # 单元测试和行为测试
OpenSiri.xcodeproj/              # Xcode 工程和共享 scheme
OpenSiri.xcworkspace/            # workspace 和 SwiftPM 集成
scripts/release/                # 发布、签名、打包和 appcast 流程
```

## 运行时边界

- 应用入口和共享状态位于 `OpenSiri/App` 及相关的 Swift 功能模块中。
- 翻译服务提供商在 `OpenSiri/Swift/Service` 下实现特定服务的请求和响应解析。
...
主要的 Xcode 入口是使用 `OpenSiri` scheme 的 `OpenSiri.xcworkspace`。构建和
