...
public class AppleOCREngine: NSObject {
    ...
    /// Recognizes text and returns the raw observations, skipping line merging.
    ///
    /// ``recognizeText(image:language:requiresAccurateRecognition:)`` merges observations into a
    /// single reading-order string, which discards the per-line geometry. Callers that classify
    /// text by where it sits — such as telling a chat's left bubbles from its right ones — need
    /// the observations themselves, each with its normalized bounding box and confidence.
    ///
    /// - Parameters:
    ///   - image: The image to recognize.
    ///   - language: Preferred recognition language, or `.auto` to let Vision decide.
    /// - Returns: Observations in Vision's own order, with bottom-left normalized coordinates.
    func recognizeObservations(
        image: NSImage,
        language: Language = .auto
    ) async throws
        ...
        let observations = try await performVisionOCR(on: cgImage, language: language)
        logInfo("Recognized \(observations.count) observations (\(language))")
        return observations
    }
    ...
    /// A mapper to convert between OpenSiri's `Language` enum and Apple's language identifiers.
