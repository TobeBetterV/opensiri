...
struct MenuItemView: View {
    ...
    var body: some View {
        ...
        .task {
            latestVersion = await fetchRepoLatestVersion(EZGithubRepoOpenSiri)
        }
    }
    ...
    private var versionString: String {
        let defaultLabel = "OpenSiri  \(currentVersion)"
    ...
    @ViewBuilder private var versionItem: some View {
        Button(versionString) {
            guard let versionURL = URL(string: "\(EZGithubRepoOpenSiriURL)/releases") else {
                return
            }
            openURL(versionURL)
        }
    ...
    @ViewBuilder private var helpItem: some View {
        Menu("Help") {
            Button("Feedback") {
                ...
                guard let versionURL = URL(string: "\(EZGithubRepoOpenSiriURL)/issues") else {
                    return
                }
                openURL(versionURL)
            }
    ...
    private func exportLogAction() {
        ...
        let zipPath = downloadDirectory.appendingPathComponent("OpenSiri log \(dataString).zip")
