    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Group {
                    LabeledContent {
                        ExcludedLanguagesMenu(selection: $autoShowQueryIconExcludedLanguages)
                    } label: {
                        Text("setting.advance.auto_show_query_icon.condition.languages")
                    }
...
// MARK: - ExcludedLanguagesMenu

/// Multi-select menu listing the languages that suppress the auto query icon.
///
/// An empty selection means no language is excluded, so the icon shows for every language.
private struct ExcludedLanguagesMenu: View {
    // MARK: Internal

    @Binding var selection: Set<Language>
    ...
    var body: some View {
        Menu {
            ...
            if !selection.isEmpty {
                Divider()
                Button("setting.advance.auto_show_query_icon.condition.languages.clear") {
                    selection.removeAll()
                }
            }
        } label: {
    ...
    /// Describes the current selection: none, the names themselves, or a count once the
    /// list grows too long to fit the menu label.
    private var summary: String {
        let selected = Language.allAvailableOptions.filter { selection.contains($0) }
        guard !selected.isEmpty else {
            return String(localized: "setting.advance.auto_show_query_icon.condition.languages.none")
        }
        guard selected.count <= 2 else {
            return String(
                localized: "setting.advance.auto_show_query_icon.condition.languages.count \(selected.count)"
            )
        }
        return selected.map { "\($0.flagEmoji) \($0.localizedName)" }.joined(separator: ", ")
    }
