...
class Screenshot: NSObject {
    ...
    public func startCapture(completion: @escaping (NSImage?) -> ()) {
        ...
        if !hasScreenCapturePermission {
            if !hasRequestedPermissionThisLaunch {
                ...
                /**
                 ...
                 The flag is deliberately per-launch rather than persisted: this call is what registers
                 the app in the Screen Recording list, so a persisted flag would permanently suppress it
                 once the TCC record goes away on its own — after `tccutil reset`, a re-signed build, or
                 a bundle identifier change. The app would then only ever show its own alert, and could
                 never reappear in System Settings for the user to grant.
                 */
    ...
    /// Whether this launch already asked the system for screen capture access.
    ///
    /// Resets on every launch so a relaunch can always re-register the app with TCC.
