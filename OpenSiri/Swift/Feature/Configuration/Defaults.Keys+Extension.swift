...
extension Defaults.Keys {
    ...
    /// Languages that suppress the query icon. Empty means every language shows it.
    ///
    /// Uses a new key rather than the former singular
    /// `EZConfiguration_kAutoShowQueryIconExcludedLanguageKey`, whose stored value is a bare
    /// string and cannot decode as a set.
    static let autoShowQueryIconExcludedLanguages = Key<Set<Language>>(
        "EZConfiguration_kAutoShowQueryIconExcludedLanguagesKey",
        default: []
    )
    ...
    static let clickQuery = Key<Bool>("EZConfiguration_kClickQueryKey", default: true)
