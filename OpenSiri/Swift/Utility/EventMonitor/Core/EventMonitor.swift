...
//            log("keyCode: \(event.keyCode)")
    ...
    private func shouldShowAutoQueryIcon(for text: String) -> Bool {
        ...
        guard !excludedLanguages.isEmpty else {
            // No language excluded, show the icon for every language and skip detection.
            return true
        }
        ...
        logInfo(
            "detected language: \(detectedLanguage), excluded languages: \(excludedLanguages.map(\.rawValue).sorted()), shouldShowAutoQueryIcon: \(shouldShowAutoQueryIcon)"
        )
