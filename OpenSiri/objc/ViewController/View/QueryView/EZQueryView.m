...
/// Clickable thumbnail of the latest conversation capture. Hidden when there is no capture.
@property (nonatomic, strong) NSButton *captureThumbnailButton;

/// Bottom-left chip showing how the Smart Screenshot capture was recognized.
@property (nonatomic, strong) EZButton *smartScreenshotChip;
@property (nonatomic, strong, nullable) NSMenu *smartScreenshotMenu;
/// Measured width of the chip's title; 0 collapses the chip out of the row.
@property (nonatomic, assign) CGFloat smartScreenshotChipWidth;
...
- (void)setup {
    ...
    EZButton *smartScreenshotChip = [[EZButton alloc] init];
    self.smartScreenshotChip = smartScreenshotChip;
    [self addSubview:smartScreenshotChip];
    smartScreenshotChip.title = @"";
    smartScreenshotChip.hidden = YES;
    smartScreenshotChip.cornerRadius = 10;
    // Match EZDetectLanguageButton so the two chips read as one row.
    [smartScreenshotChip executeLight:^(EZButton *chip) {
        chip.backgroundColor = [NSColor mm_colorWithHexString:@"#E8E8E8"];
        chip.backgroundHoverColor = [NSColor mm_colorWithHexString:@"#DCDCDC"];
        chip.backgroundHighlightColor = [NSColor mm_colorWithHexString:@"#CCCCCC"];
    } dark:^(EZButton *chip) {
        chip.backgroundColor = [NSColor mm_colorWithHexString:@"#3D3E3F"];
        chip.backgroundHoverColor = [NSColor mm_colorWithHexString:@"#47494A"];
        chip.backgroundHighlightColor = [NSColor mm_colorWithHexString:@"#585A5C"];
    }];

    mm_weakify(self);
    [smartScreenshotChip setClickBlock:^(EZButton *_Nonnull button) {
        mm_strongify(self);
        [self showSmartScreenshotMenu];
    }];

    NSButton *captureThumbnailButton = [[NSButton alloc] init];
    self.captureThumbnailButton = captureThumbnailButton;
    [self addSubview:captureThumbnailButton];
    captureThumbnailButton.bordered = NO;
    captureThumbnailButton.imageScaling = NSImageScaleProportionallyUpOrDown;
    captureThumbnailButton.imagePosition = NSImageOnly;
    captureThumbnailButton.title = @"";
    captureThumbnailButton.hidden = YES;
    captureThumbnailButton.toolTip = NSLocalizedString(@"reply_assist.thumbnail.tooltip", nil);
    captureThumbnailButton.target = self;
    captureThumbnailButton.action = @selector(captureThumbnailClicked:);
...
- (CGFloat)heightOfQueryView {
    return [self heightOfTextView] + EZQueryViewExceptInputViewHeight + [self heightOfCaptureThumbnail];
}

/// Extra height the thumbnail strip adds, or 0 when there is no capture.
- (CGFloat)heightOfCaptureThumbnail {
    if (!self.captureThumbnailImage) {
        return 0;
    }
    return EZCaptureThumbnailHeight_40 + EZCaptureThumbnailBottomPadding_6;
}

- (void)setCaptureThumbnailImage:(nullable NSImage *)captureThumbnailImage {
    if (_captureThumbnailImage == captureThumbnailImage) {
        return;
    }
    _captureThumbnailImage = captureThumbnailImage;

    self.captureThumbnailButton.image = captureThumbnailImage;
    self.captureThumbnailButton.hidden = captureThumbnailImage == nil;

    [self setNeedsUpdateConstraints:YES];
}

- (void)captureThumbnailClicked:(NSButton *)sender {
    if (self.captureThumbnailClickBlock) {
        self.captureThumbnailClickBlock();
    }
}

#pragma mark - Smart Screenshot chip

- (void)setSmartScreenshotModeName:(nullable NSString *)smartScreenshotModeName {
    _smartScreenshotModeName = [smartScreenshotModeName copy];

    if (!smartScreenshotModeName.length) {
        self.smartScreenshotChipWidth = 0;
        [self.smartScreenshotChip setAnimatedHidden:YES];
        [self setNeedsUpdateConstraints:YES];
        return;
    }

    // Same "Detected <value>" shape as EZDetectLanguageButton: gray prefix, blue value.
    NSString *detectedTitle = NSLocalizedString(@"detected", nil);
    NSString *fullTitle = [NSString stringWithFormat:@"%@ %@", detectedTitle, smartScreenshotModeName];
    ...
    NSRange modeRange = NSMakeRange(detectedTitle.length + 1, smartScreenshotModeName.length);
    [attrTitle addAttributes:@{
        NSForegroundColorAttributeName : [NSColor ez_blueTitleColor],
    } range:modeRange];

    self.smartScreenshotChip.attributedTitle = attrTitle;
    self.smartScreenshotChipWidth = [attrTitle mm_getTextWidth] + 8;

    [self.smartScreenshotChip setAnimatedHidden:NO];
    [self setNeedsUpdateConstraints:YES];
}

/// Lets the user correct the recognition or open the full editor.
- (void)showSmartScreenshotMenu {
    if (!self.smartScreenshotMenu) {
        self.smartScreenshotMenu = [NSMenu new];
    }
    NSMenu *menu = self.smartScreenshotMenu;
    [menu removeAllItems];

    NSMenuItem *conversationItem =
        [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"smart_screenshot.mode.conversation", nil)
                                   action:@selector(selectConversationMode:)
                            keyEquivalent:@""];
    conversationItem.target = self;
    conversationItem.state = self.smartScreenshotUsingConversation ? NSControlStateValueOn : NSControlStateValueOff;
    [menu addItem:conversationItem];

    NSMenuItem *plainTextItem =
        [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"smart_screenshot.mode.plain_text", nil)
                                   action:@selector(selectPlainTextMode:)
                            keyEquivalent:@""];
    plainTextItem.target = self;
    plainTextItem.state = self.smartScreenshotUsingConversation ? NSControlStateValueOff : NSControlStateValueOn;
    [menu addItem:plainTextItem];

    [menu addItem:[NSMenuItem separatorItem]];

    NSMenuItem *editItem =
        [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"smart_screenshot.action.edit", nil)
                                   action:@selector(editSmartScreenshotResult:)
                            keyEquivalent:@""];
    editItem.target = self;
    [menu addItem:editItem];

    [menu popUpBelowView:self.smartScreenshotChip];
}

- (void)selectConversationMode:(NSMenuItem *)sender {
    if (self.smartScreenshotModeChangeBlock) {
        self.smartScreenshotModeChangeBlock(YES);
    }
}

- (void)selectPlainTextMode:(NSMenuItem *)sender {
    if (self.smartScreenshotModeChangeBlock) {
        self.smartScreenshotModeChangeBlock(NO);
    }
}

- (void)editSmartScreenshotResult:(NSMenuItem *)sender {
    if (self.smartScreenshotEditBlock) {
        self.smartScreenshotEditBlock();
    }
}
...
- (void)updateConstraints {
    ...
    // Sits before the language chip: while language detection is re-running its chip goes
    // blank, and trailing that gap reads better than leaving a hole mid-row.
    // Collapses to zero width and zero spacing when hidden, so the language chip keeps its
    // original position.
    BOOL hasSmartScreenshotChip = self.smartScreenshotChipWidth > 0;
    [self.smartScreenshotChip mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.textCopyButton.mas_right).offset(hasSmartScreenshotChip ? 6.5 : 0);
        make.centerY.equalTo(self.textCopyButton).offset(-0.5);
        make.height.mas_equalTo(20);
        make.width.mas_equalTo(self.smartScreenshotChipWidth);
    }];

    [self updateDetectButton];


    BOOL hasCaptureThumbnail = self.captureThumbnailImage != nil;

    [self.captureThumbnailButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.left.offset(EZAudioButtonLeftMargin_6);
        make.height.mas_equalTo(hasCaptureThumbnail ? EZCaptureThumbnailHeight_40 : 0);
        // Keep the strip proportional to the capture so the preview is not distorted.
        make.width.mas_lessThanOrEqualTo(EZCaptureThumbnailHeight_40 * 2);
    }];

    [self.scrollView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.right.inset(0);
        if (hasCaptureThumbnail) {
            make.top.equalTo(self.captureThumbnailButton.mas_bottom).offset(EZCaptureThumbnailBottomPadding_6);
        } else {
            make.top.inset(0);
        }
...
- (void)updateDetectButton {
    ...
    [self.detectButton mas_updateConstraints:^(MASConstraintMaker *make) {
        // Anchored to the recognition chip, which collapses to the copy button's edge when absent.
        make.left.equalTo(self.smartScreenshotChip.mas_right).offset(6.5);
        make.centerY.equalTo(self.textCopyButton).offset(-0.5);
...
- (void)highlightAllLinks {
    BOOL isOpenSiriSchema = [self.schemeParser isOpenSiriScheme:self.textView.string];
    if (isOpenSiriSchema) {
        return;
    }
