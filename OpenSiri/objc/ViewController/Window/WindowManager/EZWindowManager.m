...
/// For easy debugging, when OpenSiri is running in debug mode, we don't show OpenSiri release App.
- (BOOL)hasOpenSiriRunningInDebugMode {
    BOOL isDebugRunning = [self isAppRunningWithBundleId:EZDebugBundleId];
    NSString *bundleId = [[NSBundle mainBundle] bundleIdentifier];
    BOOL isReleasedOpenSiri = [bundleId isEqualToString:EZBundleId];
    if (isDebugRunning && isReleasedOpenSiri) {
        MMLogInfo(@"OpenSiri is running in debug mode, so do not show release App.");
        return YES;
    }
    return NO;
}
