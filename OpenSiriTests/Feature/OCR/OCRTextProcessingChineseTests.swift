...
struct OCRTextProcessingChineseTests {
    ...
    func testOCRChineseMixedScript() {
        ...
        访问网站：https://opensiri.app 或 opensiri.app  了解更多信息。
        """
        ...
        #expect(normalizedText.contains("元。"))
        #expect(normalizedText.contains("https://opensiri.app")) // Preserve URLs

        // Check spacing normalization
        #expect(!normalizedText.contains("   ")) // No triple spaces
        #expect(normalizedText.contains("OpenSiri翻译")) // No extra space in mixed text
    }
